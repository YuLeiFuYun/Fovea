...
- **接受依据：** 0a/0b 分层、确定性合并门、外部环境完成证据和文档冻结已由脚本与 CI workflow 执行。
...
普通 PR 的 required gate 不依赖公共互联网：协议、弱网、截断、错误 MIME、超时、重定向和状态码通过 loopback origin 与固定语料可重复验证。公共 HTTPS、多 origin、系统 DNS/代理/VPN/TLS/CDN 行为属于计划任务、手动实验以及 `0b`/`release` 完成证据。

外部实验失败必须保留原始报告，但第三方服务瞬时 5xx、限流或不可达不得伪装成代码回归。反过来，确定性门通过也不能替代真实网络证据；两类门分别回答“代码是否回归”和“外部环境假设是否仍成立”。
