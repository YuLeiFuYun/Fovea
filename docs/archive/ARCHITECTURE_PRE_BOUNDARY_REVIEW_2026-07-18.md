...
- `docs/adr/`：局部决策及其证据。
- `docs/specifications/`：可执行规范，包括缓存/身份语义、HTTP 一致性、调度、并发所有权、基准、安全默认、SwiftUI 状态机和平台配置。
...
- 持久 key canonical encoding 与跨进程 golden vectors；
- 有界传输和 staging；
...
当实验增强或新 codec 证明需要时，再升级 governor，而不是预先冻结八维最优控制 API。各平台初始预算与 CI 分层见 `docs/specifications/platform-profiles.md`。Swift 并发所有权、ByteStream fan-out、MainActor 和 `@unchecked Sendable` 约束见 `docs/specifications/concurrency-contracts.md`。
...
2. 定义并测试 FetchVariantKey → FetchExecutionKey → RepresentationRecord → ContentID → DecodeKey → RenderKey；建立持久 key golden vectors。
...
7. 验证 schema/generation 升级、namespace revoke/commit 竞态和共享任务优先级重算。
8. 身份、HTTP、auth 隔离测试优先于复杂缓存算法。
9. 更新 Reality Gap ADR，决定现有代码逐项复用或重写。
