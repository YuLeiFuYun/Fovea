...
- `docs/adr/`：局部决策及其证据。
- `docs/specifications/`：可执行规范，包括缓存/身份语义、基准、安全默认、SwiftUI 状态机和平台配置。
...
- FetchVariantKey / RepresentationRecord / ContentID 基本正确性测试；
- 有界传输和 staging；
- W1-W3 基准骨架与 Nuke、Kingfisher 适配器；
- 安全 namespace 与登出清理骨架；
- 最小诊断事件。
...
- RFC 9111 的 freshness、validator、`Vary`、304 和核心认证语义；
- 取消、优先级和提交边界；
...
- 目标像素未知时可以按显式策略显示，但默认不把全尺寸结果写入 RenderedMemory；
- UI 复用会取消旧订阅；
...
| single-flight 注册与订阅 | FoveaCore | 仅内存任务表 |
| URLSession/URLCache 配置 | FoveaTransport/FoveaHTTP composition root | 不重复持久化 |

完整状态机和 property-test 清单见 `docs/specifications/cache-semantics.md`。
...
`SelectRepresentation` 是受控的固定插槽，不意味着引入通用 DAG。v1 只要求单候选 no-op 和简单 srcset；复杂 manifest/学习选择器仍属 Capability Slot 或 Experimental。
...
当实验增强或新 codec 证明需要时，再升级 governor，而不是预先冻结八维最优控制 API。各平台初始预算与 CI 分层见 `docs/specifications/platform-profiles.md`。
...
SwiftUI 行为不能只是 UIKit wrapper 的附录。规范状态机、request token、迟到结果和 transition 规则见 `docs/specifications/swiftui-image-state.md`。
...
“显著”由 `docs/specifications/benchmark-workloads.md` 中的 provisional 数值门限定义，而不是评审时临时解释。W3 的泄漏和污染指标是不可交换的硬门禁；W1/W2 的性能收益必须同时满足回归护栏。门限只能通过 ADR 调整，且不得在查看目标实现结果后追溯改写。
