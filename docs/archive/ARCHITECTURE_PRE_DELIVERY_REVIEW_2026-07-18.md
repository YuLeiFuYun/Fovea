...
如果 Fovea 不能在至少两个 canonical workload 上相对成熟竞品产生可复现的净收益，就不应继续扩大公共 API。具体竞品现状、未知项与验收 workload 见 `docs/COMPETITIVE_CONTRACTS.md`。
...
- `docs/COMPETITIVE_CONTRACTS.md`：只记录经来源或 Phase 0 适配器验证的竞品能力与 Fovea 验收契约。
...
Phase 0 只证明最小闭环可运行、可测量、可与竞品对照，不形成长期兼容承诺：
...
- FetchVariantKey / FetchExecutionKey / RepresentationRecord / ContentID 基本正确性测试；
- 不可变 PipelineConfiguration 与无全局可变 registry；
- 持久 key canonical encoding、opaque physical blob locator 与跨进程 golden vectors；
- 有界传输和 staging；
- W1-W3 基准骨架与 Nuke、Kingfisher 适配器；
- HTTP external conformance harness（RFC 向量、cache-tests.fyi、适用 WPT）；
- 安全 namespace、generation revoke 与登出清理骨架；
- 最小诊断事件。

通过 Phase 0 只意味着可以继续建设 Core v1，不代表公共 API 已稳定。

### 3.2 Core v1 Candidate

Phase 0 通过后才进入此层，目标是形成首个可发布候选：
...
CDN 重写、鉴权适配和 locator 解析必须在 FetchVariantKey 冻结前完成。认证材料本身不以明文进入 key；使用稳定、不可逆的 `AuthorizationContextID` 与 security namespace 表达主体和授权代际。完整构造样例见 `docs/specifications/cache-semantics.md`。
...
Fovea 不宣称实现通用代理缓存。Core v1 只承诺版本化的 private image cache profile；对未支持的 method/status/directive 选择不持久化或回源，而不是猜测语义。Phase 0 以 GET、200/304、显式 freshness、Age、validator、Vary、auth/no-store 为核心；206/If-Range 与 stale 扩展按 profile capability 推进。
...
动画属于 Phase 2/Experimental，不阻塞 Phase 0。它使用独立的 animation asset、frame cache 和 cost model，默认采用受控 decode window，不把所有帧无界解码进内存。frame timing、loop、disposal、可见性、后台和 Reduce Motion 策略见 `docs/specifications/animation-policy.md`。
...
SwiftUI 行为不能只是 UIKit wrapper 的附录。规范状态机、request token、迟到结果和 transition 规则见 `docs/specifications/swiftui-image-state.md`；points/scale 到 target pixels、布局未定和动态 resize 规则见 `docs/specifications/target-geometry.md`。
...
- `no-store` 不进入任何跨请求可复用缓存；只允许当前任务/UI 瞬态持有；
- 认证响应不进入共享 namespace；
...
“显著”由 `docs/specifications/benchmark-workloads.md` 中的 provisional 数值门限定义，而不是评审时临时解释。W3 的泄漏和污染指标是不可交换的硬门禁；W1/W2 的性能收益必须同时满足回归护栏。W3 还必须通过 Private Image Cache Profile 的 required external conformance corpus。门限和 profile 分类只能通过 ADR 调整，且不得在查看目标实现结果后追溯改写。
...
没有可观测性，不允许宣称策略更优。事件必须版本化、使用随机 RequestTraceID/SharedTaskID，并禁止输出 URL、ContentID、PhysicalBlobID、原始 namespace 或凭证。observer 使用有界异步 sink，阻塞/失败不能影响请求。详见 `docs/specifications/diagnostics-contract.md`。
...
2. 定义并测试 FetchVariantKey → FetchExecutionKey → RepresentationRecord → ContentID → DecodeKey → RenderKey；建立持久 key golden vectors、opaque physical locator 和不可变 pipeline configuration。
...
6. 接入 RFC 向量、cache-tests.fyi 和适用 WPT 的外部 HTTP conformance harness。
7. 验证 schema/generation 升级、namespace revoke/commit 竞态、共享任务优先级重算、cache hard limit 与 active lease。
8. 固化错误/回退、diagnostics schema 和 resource permit 的 deterministic tests。
9. 身份、HTTP、auth 隔离测试优先于复杂缓存算法。
10. 更新 Reality Gap ADR，决定现有代码逐项复用或重写。
...
并行不等于抢占主线：FoveaLab 不得修改 Core 公共协议、不得阻塞 Phase 0 PR、不得进入默认依赖图。需要新增 capability slot 时，必须先提交独立 ADR，证明现有接缝无法表达。
...
- **Phase 0 Gate 极小且可裁决，Core v1 Candidate 严格，Stable Core 只包含已验证承诺；**
