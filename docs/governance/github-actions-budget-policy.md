...
- `full` is mutually exclusive with `identity` and is reserved for one locally qualified, immutable release candidate. It includes the complete product, Workbench, mutation, rollback, privacy, and evidence gates, with a 120-minute hard ceiling.

The former 60-minute ceiling is a measured rejected limit, not a target. Protected run `33487750055` was cancelled after exceeding exactly 1h0m0s while still inside the product qualification step, and the latest complete local qualification already required about 75.6 minutes before the hosted rollback and evidence steps. The 120-minute ceiling preserves a finite budget while allowing the current governed gate to finish; exceeding it remains a release-blocking infrastructure failure.
...
4. dispatch `full` once, with an exact `base_commit` when rollback evidence is required; it may consume at most 120 runner-minutes and does not repeat identity;
