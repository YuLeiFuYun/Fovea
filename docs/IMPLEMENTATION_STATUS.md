...
Fovea 当前生产组件 pin 以 `docs/project-memory/component-pins.json` 为权威：ImageCraft `c16a868f1a1c0ed6b1a916ad082f762969ac5a7e` / `0.1.0-alpha.8`，Akashic `0376b960ec8abe54f2d4a9d7d66e97f395215eaf` / `0.1.0-alpha.7`。ImageCraft alpha.8 发布 bounded frame-window / whole-track animation cost contract，FoveaSystem 已编译生产 `ImageCraftAnimationPlaybackPreparer` 并把这些成本并入宿主全局 animation admission；Akashic alpha.7 发布动态 global-unassigned memory budget 与精确 eviction report，Fovea rendered-cache 会按实际 eviction 同步 residency。历史 alpha.4/alpha.5 的 progressive、CacheLab、fault/resource 证据继续绑定原 revision，不因当前 pin 前移而自动迁移结论。

当前发布收口状态：Fovea 公共仓库根已存在，`main` 仍为 `f4b320b7f995bc565f0ca5ef053a5cd5bfb3a3c7`，`0.1.0-alpha.1` 尚未发布。候选 `6cf6175c3c47de64fa9d60341de7e5cd71cd29b7` 已通过 exact 本地 qualification 与 hosted identity run `33501353676`，但 hosted full run `33505961407` 两次都在 Workbench `ui-tests-core-shard-1` 失败；同一 exact candidate 的本地 qualification 与两次额外 shard 复现均为 5/5 green。`d191790f` 已加入 terminal XCTest failure 识别、case-state 摘要和有限 wrapper-exit grace，避免真实 terminal failure 再被 240 秒静默 watchdog 误分类。当前开发线还包含 alpha.8/alpha.7 生产集成，因此最终 candidate 必须先完成本页/机器状态同步，再从精确 clean commit 重新执行本地 qualification、fresh identity 与 full protected verification。GitHub `Fovea Verification` 当前为 active；账户剩余 Actions 预算在现有 token 下不可观测，不能声称充足或耗尽。
...
- 双组件候选组合门现使用 source identity v2 与 schema-4 组合报告：Fovea 不执行候选身份工具，而是独立验证外部身份文档、完整覆盖范围、精确排除、逐文件字节摘要与可执行位，再从已验证字节物化快照。候选 manifest、`package edit`、依赖检查和完整测试均在 `FOVEA-COMPONENT-CANDIDATE-SANDBOX-V1` macOS Seatbelt 中执行：网络、宿主源码读取、宿主写逃逸、候选快照写入及隔离 Fovea 源码写入由七个实际探针验证为拒绝，只有专用 State 写入成功；Fovea 不可变源码前后摘要一致，测试后两个组件身份再次一致。当前 Git-free 候选对（Akashic `5887e2facb07` / 84 文件、ImageCraft `045aa7013b19` / 94 文件）分别通过完整 release-readiness 与 clean-copy，隔离组合回归通过 478/478；六类身份负向样例仍在构建前被拒绝。该结果绑定 deprecated SwiftPM `native` build system，候选仍可读取隔离 Fovea 源码，因此不声明双向保密、默认 `swiftbuild` 等价或 release qualification。这组 sandbox 结果是历史 source-bound campaign：当时相同候选字节分别发布为 Akashic `2715f23d50b5`（`0.1.0-alpha.5`）与 ImageCraft `e147b349d4ff`（`0.1.0-alpha.4`），不得把报告 revision 直接改写成当前组件版本。当前生产 pin 已前移到 ImageCraft alpha.8 / Akashic alpha.7，conformance manifests 与生产 adapter 已随 pin 更新；Fovea protected release qualification 仍待最终 clean candidate。
...
- 完整 workload 路线是 W1-W15。W1-W3 已在干净提交 `5a836c43566a` 下完成六项 A 级 headless、30 个规格的 Simulator 结构合同诊断；Fovea 与 Apple Native 硬失败为 0，四个外部库共稳定暴露 8 个 W3 语义缺口。该诊断未通过宿主静默门，只能用于正确性与能力边界，不能用于延迟、吞吐或资源排名。W7 已具备版本化 1,000 请求 runner，W10 已具备 Apple AsyncImage/Fovea 配对 SwiftUI surface。W4 渐进 JPEG 的生产事件生命周期已实现，W5 已具备 ImageCraft alpha.8 GIF/APNG production adapter 与宿主播放/资源准入；两者仍缺 clean preregistered comparative、稳定物理设备和 protected release evidence，因此 claim activation 继续关闭。W6 完整中断恢复仍是 capability gap，其余 workload 按 registry 状态推进。`Adaptive Representation` 是辅助 X1，不占用 W4。
- Comparative Lab 的 A 级矩阵现为 Apple URLSession + URLCache + ImageIO、Apple AsyncImage、Nuke 13.0.6、Kingfisher 8.11.0、SDWebImage 5.21.7、PINRemoteImage `releases/p14.31` 与 Fovea。Apple headless adapter 已改为真正的 URL Loading System 协议缓存：`URLSessionConfiguration.urlCache` + `.useProtocolCachePolicy`，通过 `URLSessionTaskMetrics.resourceFetchType` 观测来源，不再手工查找或写入缓存；真实 loopback 测试证明首次 network、清空解码内存后第二次 disk，且 origin 只收到一次请求。W2 六项 headless 与 AsyncImage/Fovea SwiftUI surface 现共用同一 `NWListener` HTTP origin。六项 headless 实现绑定统一 request/target/cancel/result 合同；Apple AsyncImage 与 FoveaResponsiveImage 绑定同一 loopback origin、trace、采样器和配对 SwiftUI surface。AlamofireImage 4.4.0 降为 B 级补充，不能替代任何 A 级槽位。七项本地 target 均已构建。W7 V6 校准先揭示取消墓碑会误伤取消前已开始但较晚到达 registry 的 survivor cohort；admission-aware replacement cohort 修复已通过 SCHED-PT-021/022。V7 随后修复取消结果归一化、取消预热 actor hop 前的同步 launch claim，并把内部八次 grant 上限与八槽并发下十五次黑盒 origin-start 上限分离。但进一步重复证明 V7 的 single-flight 准备边界无效：六个主要 adapter 都在 `makeLoad` 内启动原生工作，runner 却串行等待 512 次创建，固定 750 ms 延迟不能保证每组 32 个 subscriber 在首个响应完成前全部存在。因此 V7 shared-origin 结果和对 SDWebImage 第 17 次请求的归因均已归档为混杂；Nuke 独立 priority 子轨迹的 31 次 newer starts 仍保留。V8 在构造全部 512 个 shared load 前关闭仅作用于 `/w7/shared` 的响应交付栅栏，保持请求创建、原生 single-flight enrollment、取消与优先级传播可运行，构造完成后再释放，并新增实际 waiter 硬门。tree-bound dirty 因果诊断在 iOS 27.0 build `24A5355p` 上 4/4 均记录八个实际栅栏 waiter、16 次 shared origin、256 cancelled、0 failed、硬失败 0；此前未绑定 W7 DerivedData/Simulator 的跨库 smoke 已降为排障证据，不进入 V8 校准。V8 clean 六库结构校准尚未执行，不能形成排名。当前六项 headless 已在同一干净 source-tree digest 下完成 W1-W3 的 30 规格功能诊断，并对 W3 做了每库 3 次重复：Apple Native 与 Fovea 3/3 全部通过；Nuke、SDWebImage 的 no-store、revoke race、跨 origin Authorization 三项缺口，以及 Kingfisher、PINRemoteImage 的跨 origin Authorization 缺口均 3/3 复现。Apple AsyncImage 与 FoveaResponsiveImage 的配对 SwiftUI surface 也已在干净提交 `2decb76b320e` 下完成 5 个平衡顺序 block、3 个 workload、30 次结构运行，全部生命周期与可见性硬门通过；W1、W10 请求数差异只作为 surface 行为记录，不解释为性能优势。六库与 SwiftUI 静默宿主性能 aggregate、真机正式证据仍需完成。
...
- Cache Lab V4 保留 PINDiskCache 原生 D1 为描述项，并以独立 D5 proof wrapper 做同语义比较。V2 最终运行通过 12/13 项且存在计时边界不一致；V3 虽预生成 corpus，但计划声明 20 rounds、实现只执行 1 round，且最终仍有三个支配失败。V4 真正执行 20 个 fresh-cache rounds，将磁盘 p99 扩展到 8 轮、1536 个同步样本，并固定 8 分片。20 个独立 clean process block 的正式 campaign 在 13/13 个适用比较中跨过 20% 支配下界，正确性与统计失败均为 0。该正式 campaign 精确绑定 Akashic `2715f23d50b5` / `0.1.0-alpha.5`，继续作为历史有效证据保留；Fovea 当前生产 pin 已是 Akashic `0376b960ec8abe54f2d4a9d7d66e97f395215eaf` / `0.1.0-alpha.7`，其动态 global-unassigned budget 与 eviction-report 实现不能继承 alpha.5 的 V4 性能结论。当前 alpha.7 需要新的 clean formal campaign、protected Fovea evidence 与物理设备资源证据。
...
- 当前开发线基线已是 clean committed history；本次 current-state 同步完成后仍必须形成新的 clean final candidate，再绑定 HEAD/tree digest 执行 qualification。历史 dirty/edited 工件继续按原身份保留，不能冒充 final commit 或 trusted CI 证据。
...
| 内存与磁盘缓存 | 已实现 | 当前精确 pin 为 Akashic `0.1.0-alpha.7`；Fovea 使用动态 global-unassigned memory budget，并以 Akashic 精确 eviction report 同步 rendered-cache residency。历史 V4 13/13 性能证据仍只绑定 alpha.5，当前 alpha.7 的 clean formal/physical/protected 性能资格仍待完成 |
...
| 动画播放 | 开发线生产集成已实现 | FoveaSystem 已编译 ImageCraft alpha.8 `ImageCraftAnimationPlaybackPreparer`，支持授权 GIF/APNG 准备、帧窗口 decode、caller-versioned timing 与宿主全局资源准入；当前入口仍是 package-level，普通稳定公开动画 API、同语义 comparator、物理 iOS 资源/能耗和 protected release qualification 尚未完成 |
