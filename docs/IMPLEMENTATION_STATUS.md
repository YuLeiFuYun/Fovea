...
- Comparative Lab 的 A 级矩阵现为 Apple URLSession + URLCache + ImageIO、Apple AsyncImage、Nuke 13.0.6、Kingfisher 8.11.0、SDWebImage 5.21.7、PINRemoteImage `releases/p14.31` 与 Fovea。Apple headless adapter 已改为真正的 URL Loading System 协议缓存：`URLSessionConfiguration.urlCache` + `.useProtocolCachePolicy`，通过 `URLSessionTaskMetrics.resourceFetchType` 观测来源，不再手工查找或写入缓存；真实 loopback 测试证明首次 network、清空解码内存后第二次 disk，且 origin 只收到一次请求。W2 六项 headless 与 AsyncImage/Fovea SwiftUI surface 现共用同一 `NWListener` HTTP origin。六项 headless 实现绑定统一 request/target/cancel/result 合同；Apple AsyncImage 与 FoveaResponsiveImage 绑定同一 loopback origin、trace、采样器和配对 SwiftUI surface。AlamofireImage 4.4.0 降为 B 级补充，不能替代任何 A 级槽位。七项本地 target 均已构建。W7 V6 校准先揭示取消墓碑会误伤取消前已开始但较晚到达 registry 的 survivor cohort；admission-aware replacement cohort 修复已通过 SCHED-PT-021/022。当前 W7 V7 又暴露两项真实问题：adapter 将取消后无 final 的正常流终止误报为 unexpected failure，以及取消预热在 actor hop 前未去重，导致四个全员取消组最多额外重取。现已改为取消优先归一化、动态 fetch 交接租约与同步 launch claim；Fovea 单库 1,000 请求校准通过全部硬门，结果为 16 次 shared origin、256 cancelled、0 failed、origin 峰值 8。V6 还证明内部 permit grant 顺序不能直接等同于并发 URLSession 的 origin start 顺序；V7 保持内部八次 grant 上限，并将跨库黑盒上限按八槽并发严格推导为十五。当前六项 headless 已在同一干净 source-tree digest 下完成 30 规格功能诊断，并对 W3 做了每库 3 次重复：Apple Native 与 Fovea 3/3 全部通过；Nuke、SDWebImage 的 no-store、revoke race、跨 origin Authorization 三项缺口，以及 Kingfisher、PINRemoteImage 的跨 origin Authorization 缺口均 3/3 复现。W7 V7 也已完成当前 clean-source 六库结构矩阵：Fovea、Apple Native、Kingfisher、SDWebImage 与 PINRemoteImage 在主矩阵中通过全部硬门；Nuke 的旧 background probe 被 31 个 newer origin starts 越过，超过 15 的派生上限，并在 3/3 重复中稳定复现。SDWebImage 在聚焦重复中 1/3 产生第 17 次 shared transport；该请求未交付额外字节，因异步缓存查询后尚未完成 downloader enrollment 而被立即停止，属于原生 manager 取消/合并边界，不由 adapter 添加私有 single-flight 掩盖。独立诊断路径只写入单独目录，不覆盖正式工件。Apple AsyncImage 与 FoveaResponsiveImage 的配对 SwiftUI surface 也已在干净提交 `2decb76b320e` 下完成 5 个平衡顺序 block、3 个 workload、30 次结构运行，全部生命周期与可见性硬门通过；W1、W10 请求数差异只作为 surface 行为记录，不解释为性能优势。六库与 SwiftUI 静默宿主性能 aggregate、真机正式证据仍需完成。
