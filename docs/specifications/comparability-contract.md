...
| L2 Primary portfolio | 一个有限作用域声明的少量预注册主 endpoint | 逐指标实质优越或可证明的最优下限 gate |
...
全局、无边界的 “world best” 营销声明仍被禁止。工程目标和机器证书必须绑定以下身份：
...
`dominant-within-scope` 要求：

1. 该作用域全部 L1 硬门通过；
2. 每个可比较 L2 主 endpoint 均相对每个合格对手达到预注册的实质领先幅度；默认相对幅度为 20%，有界指标使用预注册绝对幅度；
3. 唯一例外是已经达到数学或契约最优下限的指标，例如错误数为 0、泄漏数为 0、命中率为 1；该例外必须由机器可检查的下限证明支持，不能用“差异不显著”代替；
4. 任何主 endpoint 仅为等价、非劣、落后或证据不足，都会阻断支配证书；
5. 至少 80% 的可比较 L3 次 endpoint 达到实质优越，并且次 endpoint 的实质落后预算为 0；
6. 稳定系统、多真机、held-out、独立复现和全部摘要身份有效。

`best-within-scope` 仅在对应作用域的 `dominant-within-scope` 工程目标通过后生成。若全部主 endpoint 只证明等价或非劣，只能输出 `noninferior-within-scope`，不能暗示最佳。

这里禁止总分抵消：一个指标的大幅领先不能掩盖另一个可比较指标的落后。比较对象必须在结果产生前锁定；不得在看到结果后移除最强对手。
