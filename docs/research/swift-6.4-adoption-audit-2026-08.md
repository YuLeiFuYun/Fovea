...
### 7.8 Hosted 冷启动与视觉 watchdog 反例

第十个私有根在 Workbench 开始前暴露了错误的冷启动判据：`simctl boot` 返回 0 后，重复 `simctl list` 仍无法恢复目标 UDID，最终只得到 `last_state=unknown`。就绪判据现改为有界 `simctl bootstatus <UDID> -b -d`；设备列表只承担已启动快速路径和失败诊断。第十一个私有根 `49c34224fd354578e3ea5c3e1b07bb26fd30d4ba` 的 run `30690150522` 已通过该边界，两个身份 job 均绿色，主 verify 通过全部 Workbench 前置门并进入双设备视觉矩阵。

同一 run 随后暴露嵌套 watchdog 不一致：外层单设备进程只允许 600 秒，内层 `xcodebuild` 却允许 2700 秒，并把 stdout 缓冲到进程结束后才写入 family capture log。iPad family 因而连续两次被外层先行终止，无法保留内层进度；视觉命令还没有禁用 Xcode 27 自动诊断。修正后的内层视觉执行器直接流式写入 family log，以 720 秒为总界限、240 秒为无日志进展界限，并传入 `-collect-test-diagnostics never`。外层 family 只保留 1200 秒兜底，不能先于内层证据 oracle 终止任务。

本机长 UI 重放还证明了另一个基础设施缺口：240 秒 inactivity watchdog 正确终止无进展的 UI shard 后，`simctl shutdown` 可进入不可中断状态。旧通用执行器在完成有界 TERM/KILL 后又无条件调用一次 `process.wait()`，从而重新引入无限等待。当前 timeout 路径在有界清理返回后直接产生 `returncode=-1`；不可回收假进程回归要求只发生一次带 timeout 的 wait。

### 7.9 Hosted Actions 预算反例与准入控制

私有发布过程中连续根候选触发了多次 xcode-27 双架构身份与完整验证。每次失败都提供了真实的工具链、连续性、Simulator 或 UI 反例，但把昂贵 Hosted CI 当作探索循环，最终耗尽了账户配置的 100 美元 GitHub Actions 使用预算，且 Fovea 仍未形成绿色发布根。

因此 Hosted CI 被重新定义为稀缺的发布证据资源。Fovea 的远端 Verification、Live Network 与临时 Diagnostic 工作流均已手动禁用；临时诊断分支已删除。本地候选把 Verification 改为纯 `workflow_dispatch`，默认 `budget_approved: false`，并提供互斥的 `identity` 与 `full` 两档：identity 的两个架构 job 各限制 15 分钟，full 单独限制 60 分钟，不重复执行 identity。Live Network 删除每日 schedule，同样要求显式预算批准。静态门 `check-actions-budget-governance.py` 会拒绝 push、pull_request、schedule 自动触发，拒绝缺失预算批准、job timeout 或并发取消的工作流。

后续不再推送多个试探性根。先在本机完成一个精确、净化、不可变候选，跑完连续性、隐私、供应链、101 个 mutation application、组件 exact pin、Workbench 与可用 Simulator 门，再估算上限分钟数。预算恢复后只运行一次 `identity`，审核结果和剩余额度后再运行一次 `full`。预算未恢复时，缺失 Hosted run 只能记为发布阻断，不能解释为通过。

### 7.10 DocC 成功标记与进程终止不是同一事实

预算治理候选的本机非 Simulator 验证中，macOS `xcodebuild docbuild` 已在日志末尾输出 `** BUILD DOCUMENTATION SUCCEEDED **`，并生成 17 个 symbol graph 与 17 个 doccarchive；但 `xcodebuild` 子进程仍处于 `?NE` 退出态超过五分钟，父 `verify-documentation.py` 的采样栈停在 `os_waitpid -> __wait4`。终止父进程后，下一平台的 iOS docbuild 又成为孤儿进程。这说明构建成功标记、证据产物完整和进程生命周期完成是三个独立命题。

文档门现不再使用无界 `subprocess.run`。每个平台的 DocC 构建在独立进程组中执行，stdout 直接流入保留日志；总时限为 900 秒，无日志进展时限为 180 秒。达到任一边界后只执行有界 TERM/KILL 清理，写入明确 marker 并以 CLI 状态 124 返回失败，不再做第二次无界等待。工具门包含正常输出与静默子进程两条真实回归。完整候选重放再次观察到 macOS DocC 成功标记后 `xcodebuild` 停留 `?NE`，并在最后日志后的第 180 秒准确失败关闭；未生成或接受最终 documentation report。由此 watchdog 修复已验证，剩余阻断是必须通过主机恢复或重启消除的 Xcode 进程生命周期故障。

### 7.11 取消预热的 admission 谱系与语义 oracle

完整宿主重放暴露了 adaptive warmup 的两层混淆。第一，取消后的 warmup 曾创建新的 `SharedTaskAdmission.now()`；若它在取消墓碑安装后到达 SharedTaskRegistry，就会被当作真正的取消后新调用，加入已取消任务而无法生成 handoff。warmup 实际是原顶层 `events(for:)` 调用的 replacement cohort，因此现在由该调用创建唯一 admission，并贯穿前台 load 与取消 warmup。真正的新调用仍保留墓碑隔离。

第二，测试把“3 次取消”错误等同为“恰好 3 个 transport 请求”。结构化诊断观察到合法序列 `warmup-0, warmup-1, warmup-1, warmup-2, warmup-2`：三次前台尝试之外，第二和第三次取消可各启动一次独立 replacement warmup。原三-stub fixture 因此在最终 handoff 前耗尽；不同调度下请求总数又可能只有 2 或 3，固定计数既非必要也非充分条件。修正后的 fixture 覆盖 5 次最坏路径，测试等待 `finalRequest` 的 namespace/generation/execution exact handoff，并验证可见消费者前后的完整 transport URL 序列不变。该用例连续 20/20 通过；另一个确定性测试证明 exact `contains` 不提升 SIEVE 访问位，随后完整宿主套件 476/476 通过。
...
本地已证明 x86_64/macOS 14 `FoveaTests.xctest` 完整严格交叉构建，产物为单一 x86_64 Mach-O；Xcode 27 的 xctest 与平台 XCTest/Testing 运行时均包含 x86_64 slice。GitHub Actions run `30689587302` 的 x86_64 job `91341807599` 已通过 direct Rosetta xctest 精确 selector，执行 1 项、0 失败；ImageCraft/Akashic alpha.3 也已通过 xcode-27 main CI 并打不可变标签。第十一根已证明 cold boot 与双架构身份门，但 iPad 视觉 nested watchdog 仍阻止完整 verify；修正后的实时日志与有界清理必须在下一私有根通过后，才能进入 rollback/evidence。

Fovea 既有 UI 顺序缺陷已经由测试级主 actor App teardown 关闭，原五测试 shard 5/5 通过。仓库继续 private；当前 Hosted Actions 预算已耗尽且所有远端工作流已禁用。先完成一个精确本地候选，预算恢复后再以一次 identity 和一次 full 完成 required checks、rollback 与隐私验证。
