...
The numbers below are diagnostic evidence from the current dirty worktree. They are not release or
marketing claims and do not replace a clean-commit, quiet-host formal run.
...
The candidate passes the Akashic 55-test suite, public-API additive diff, six-case Apple platform matrix, full local release-readiness mechanics, and a Git-free Fovea 478-test integration. The production Fovea source switch remains reverted so the main package continues to build against its public exact Akashic pin. V2 passed 12/13 comparisons and was rejected; V3 had a declared-versus-executed round mismatch and three final failures. V4 executes twenty fresh-cache rounds, selects eight shards after explicit 16/32-shard retention counterexamples, and its twenty clean process blocks pass all 13 applicable dominance comparisons. Publication of the measured Akashic revision, an updated Fovea exact pin, and clean trusted-CI repetition remain mandatory before a release certificate.
