...
def run(
    ...
    with tempfile.TemporaryFile(mode="w+t") as output:
        ...
        try:
            ...
            # terminate_process_group is deliberately bounded. A child can remain
            # stuck in an uninterruptible kernel state even after SIGKILL; waiting
            # again here would silently reintroduce an unbounded verifier hang.
        output.flush()
        output.seek(0)
        stdout = output.read()
