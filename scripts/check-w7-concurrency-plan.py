...
def main() -> int:
    ...
    if execution.get("simulatorCalibrationIdentity") != expected_simulator_identity:
        errors.append("W7 V10 Simulator calibration identity drifted")
    expected_status = (
        "preregistered-after-v9-origin-gate-readiness-counterexample-before-formal-results"
    )
    if plan.get("schemaVersion") != 10 or plan.get("planID") != "FOVEA-W7-CONCURRENCY-V10":
        errors.append("unexpected W7 plan identity")
    if plan.get("status") != expected_status:
        errors.append("W7 V10 plan must remain preregistered before formal results")
    if plan.get("supersedes") != "Benchmarks/W7ConcurrencyLab/plans/experiment-plan-v9.json":
        errors.append("W7 V9 plan archive binding is missing")
    archived_plan = json.loads(
        (ROOT / "Benchmarks/W7ConcurrencyLab/plans/experiment-plan-v9.json").read_text()
    )
    if archived_plan.get("schemaVersion") != 9 or archived_plan.get("planID") != "FOVEA-W7-CONCURRENCY-V9":
        errors.append("archived W7 V9 plan identity drifted")
    ...
    if plan.get("comparators") != expected_comparators:
        errors.append("W7 V10 A-tier headless comparator set drifted")
    ...
    if supplemental.get("comparators") != ["AlamofireImage"] or "excluded from A-tier" not in supplemental.get("policy", ""):
        errors.append("W7 V10 B-tier supplemental policy drifted")
    ...
    if set(not_applicable) != {"Apple AsyncImage"} or "headless" not in not_applicable.get("Apple AsyncImage", ""):
        errors.append("W7 V10 must preserve AsyncImage as not applicable")
    ...
    if priority.get("blockerRequests") != 8:
        errors.append("W7 V10 must keep eight blocker requests")
    if probe.get("olderBackgroundRequests") != 1 or probe.get("newerImmediateRequests") != 31:
        errors.append("W7 V10 starvation probe shape drifted")
    ...
    if internal_bypass != 8:
        errors.append("W7 V10 internal grant-bypass bound drifted")
    if observable_bypass != 15 or observable_bypass != derived_observable:
        errors.append("W7 V10 origin-start bound must equal 8 + client budget - 1 = 15")
    if probe.get("observableBoundDerivation") !=         "internalMaximumGrantBypasses + clientConcurrentFetchBudget - 1":
        errors.append("W7 V10 observable-bound derivation is missing")
    if balanced.get("requestsPerPriority") * 4 != 448:
        errors.append("W7 V10 balanced burst shape drifted")
    if balanced.get("submissionOrder") != "round-robin-background-utility-visible-immediate":
        errors.append("W7 V10 must keep round-robin balanced arrivals")
    if 8 + 1 + 31 + balanced.get("logicalRequests", 0) != 488:
        errors.append("W7 V10 priority subtrace no longer totals 488")
    ...
    expected_barrier = {
        ...
        "waitForOriginCapacityBeforeRelease": True,
        "requiredObservedWaitersExact": 8,
    }
    if barrier != expected_barrier:
        errors.append("W7 V10 shared-response preparation barrier drifted")
    if "single-flight-preparation-capacity-exact" not in plan.get("hardChecks", []):
        errors.append("W7 V10 preparation-capacity hard check is missing")
    ...
    if single.get("adapterPreparation") != expected_adapter_preparation:
        errors.append("W7 V10 native adapter preparation contract drifted")
    if "logical-load-preparation-count-exact" not in plan.get("hardChecks", []):
        errors.append("W7 V10 logical preparation hard check is missing")
    ...
    workload_preparation_markers = [
        ...
        "metrics.w7SharedPreparationWaitCount >= 8",
    ...
    if claims.get("schemaVersion") != 10 or claims.get("registryID") != "FOVEA-W7-CONCURRENCY-CLAIMS-V10":
        errors.append("unexpected W7 claim registry identity")
    if claims.get("status") != expected_status:
        errors.append("W7 V10 claim registry must remain preregistered")
    if claims.get("supersedes") != "Benchmarks/W7ConcurrencyLab/plans/claim-families-v9.json":
        errors.append("W7 V9 claim archive binding is missing")
    archived_claims = json.loads(
        (ROOT / "Benchmarks/W7ConcurrencyLab/plans/claim-families-v9.json").read_text()
    )
    if archived_claims.get("schemaVersion") != 9 or archived_claims.get("registryID") != "FOVEA-W7-CONCURRENCY-CLAIMS-V9":
        errors.append("archived W7 V9 claim identity drifted")
    efficiency = next(
