...
def main() -> int:
    ...
    if policy.get("schemaVersion") != 3 or policy.get("claimPolicyID") != "FOVEA-STATISTICAL-CLAIM-POLICY-V3":
    ...
    if "dominant-within-scope" not in scope.get("allowedOutputs", []):
        errors.append("dominant-within-scope output is missing")
    dominance = policy.get("engineeringDominanceTarget", {})
    if dominance.get("defaultRelativeDominanceMargin") != 0.20:
        errors.append("default engineering dominance margin must remain twenty percent")
    if dominance.get("secondaryEndpointCoverageMinimum") != 0.80:
        errors.append("secondary dominance coverage must remain eighty percent")
    if dominance.get("secondaryEndpointInferiorityBudget") != 0:
        errors.append("secondary endpoint inferiority budget must remain zero")
    if "endpoint-by-endpoint" not in dominance.get("aggregateScore", ""):
        errors.append("dominance must remain endpoint-by-endpoint rather than aggregate-score based")
