...
def main() -> int:
    ...
    if outcome.get("bestClaimRule") != "best-within-scope-requires-every-applicable-L2-primary-endpoint-substantially-superior-or-optimal-floor-saturated":
    ...
    for metric, threshold in thresholds.items():
        ...
        dominance = threshold.get("dominanceRelative")
        if not isinstance(dominance, (int, float)) or dominance < 0.20:
            fail(f"dominance margin must be at least twenty percent for {metric}")
