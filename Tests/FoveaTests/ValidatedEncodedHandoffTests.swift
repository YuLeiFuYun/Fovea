...
final class TransportVerifiedEncodedHandoffTests: XCTestCase {
    ...
    func testDiskStorePurgeLinearizesAgainstAdmittedPreparation_CACHE_PT_053() async throws {
        let checkpoint = HandoffPreparationCheckpoint()
        let store = TransportVerifiedHandoffDiskStore(
            costLimit: 64,
            preparationCheckpoint: { checkpoint.reachAndWait() }
        )
        let request = try makeRequest(path: "purge-race.png")
        let generation = NamespaceGeneration(0)
        let data = Data(repeating: 0x6C, count: 8)
        let metadata = try XCTUnwrap(
            TransportVerifiedEncodedHandoffMetadata(
                record: makeRecord(
                    data: data,
        ...
        try await waitUntil("handoff preparation reached publication checkpoint") {
            checkpoint.wasReached
        }
        _ = await store.removeAllAndReport()
        checkpoint.release()

        do {
            try await insertion.value
            XCTFail("A preparation admitted before purge must not publish afterward")
        } catch is CancellationError {
            // 清理先线性化时，旧 epoch 的文件错误必须规范化为取消。
        }
...
private final class HandoffPreparationCheckpoint: Sendable {
    private let reached = DispatchGroup()
    private let released = DispatchGroup()
    ...
    var wasReached: Bool {
        reached.wait(timeout: .now()) == .success
    }
