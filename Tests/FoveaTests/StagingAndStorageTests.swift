...
final class StagingAndStorageTests: XCTestCase {
        ...
        async throws
    {
        ...
        do {
            let accumulator = try BoundedStagingAccumulator(
                maximumBytes: expected.count,
                memoryThreshold: 0,
                stagingLease: try XCTUnwrap(sessionLease)
            )
            try accumulator.append(expected)
    ...
    func testPersistentMetadataDoesNotContainPlaintextNamespace() async throws {
        ...
        let manifestObject = try storageJSONObject(Data(manifest.utf8))
        let entries = try storageDictionary(manifestObject["entries"])
        let entry = try storageDictionary(try XCTUnwrap(entries.values.first))
    ...
    func testStoreRejectsMismatchedContentID() async throws {
        let store = try await AkashicOriginalEncodedStore.open(root: try makeTemporaryDirectory())
    ...
    func testReadAccessSurvivesReopenAndControlsLRUEviction() async throws {
        ...
        _ = try await store!.commit(data: second, contentID: secondID, namespace: namespace)
        let firstBlobURL =
            root
        ...
        _ = try await store!.read(contentID: firstID, namespace: namespace)
        store = nil
        await Task.yield()
    ...
    func testLowerTotalLimitTrimsImmediatelyOnReopen() async throws {
        ...
        _ = try await initial!.commit(data: first, contentID: firstID, namespace: namespace)
        try await Task.sleep(for: .milliseconds(20))
        _ = try await initial!.commit(data: second, contentID: secondID, namespace: namespace)
        initial = nil
        await Task.yield()
    ...
    func testLowerRuntimeBlobLimitReconcilesEntriesWithoutInvalidatingManifest() async throws {
        ...
        _ = try await initial!.commit(data: data, contentID: contentID, namespace: namespace)
        initial = nil
        await Task.yield()
