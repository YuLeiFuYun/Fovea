...
final class BoundedMetadataReadTests: XCTestCase {
  func testOversizedStoreManifestsFailBeforeUnboundedRead_SEC_CASE_032() async throws {
    ...
    do {
      _ = try await OriginalEncodedStore.open(root: originalRoot)
  ...
  func testOversizedGenerationPointerFailsWithoutRewrite_SEC_CASE_032() throws {
    let root = try makeTemporaryDirectory("oversized-generation-pointer")
    _ = try StoreGenerationDirectory.open(
      root: root,
      compatibilityFingerprint: "bounded-pointer-v1"
    )
    let pointer = root.appendingPathComponent("current-generation.json")
    try FileManager.default.removeItem(at: pointer)
    try createSparseFile(at: pointer, byteCount: 128 * 1024)
    ...
    XCTAssertEqual(try fileSize(pointer), 128 * 1024)
  }
