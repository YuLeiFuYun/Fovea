...
final class SwiftUIStateTests: XCTestCase {
    func testSwiftUIPoliciesClampNonfiniteAndHostileDurations() {
        XCTAssertEqual(
            FoveaImageLoadingPolicy(placeholderDelayNanoseconds: UInt64.max)
                .placeholderDelayNanoseconds,
            60_000_000_000
        )
        XCTAssertEqual(
            FoveaImageTransitionPolicy(opacityDuration: .infinity).opacityDuration,
            0.2
        )
        XCTAssertEqual(
            FoveaImageTransitionPolicy(opacityDuration: .nan).opacityDuration,
            0.2
        )
        XCTAssertEqual(
            FoveaImageTransitionPolicy(opacityDuration: 100).opacityDuration,
            10
        )
    }
    ...
    func testZeroDelayPublishesLoadingWithoutRedundantEmpty_UI_PT_022() async throws {
        let model = FoveaImageModel()
        let loader = DelayedImageLoader(image: try decodedImage(), delayNanoseconds: 20_000_000)
        var kinds: [FoveaImagePhaseKind] = []
        let observation = model.$phase.dropFirst().sink { kinds.append($0.kind) }
        defer { observation.cancel() }

        await model.load(
            request: try request(path: "zero-delay.png"),
            loader: loader,
            policy: FoveaImageLoadingPolicy(placeholderDelayNanoseconds: 0)
        )

        XCTAssertEqual(kinds, [.loading, .success])
    }
    ...
    func testResponsiveModelResolvesAndLoadsWithSingleLifecycle_UI_PT_023() async throws {
        let model = FoveaResponsiveImageModel()
        let image = try decodedImage(width: 24)
        let loader = ImmediateImageLoader(image: image)
        ...
        await model.updateAndLoad(
            ...
            return try ImageRequest.publicImage(
                url: XCTUnwrap(URL(string: "https://example.test/fused-responsive.png")),
    ...
    func testRetryUsesNewGenerationUiPt006() async throws {
        ...
        XCTAssertEqual(model.phase.kind, .success)
        XCTAssertNotEqual(model.requestGeneration, failedGeneration)
        let requestCount = await loader.requestCount
        XCTAssertEqual(requestCount, 2)
    }

    func testCancelledSameIdentityReloadsWhenViewReappears_UI_PT_021() async throws {
        let model = FoveaImageModel()
        let loader = CancellationThenSuccessImageLoader(image: try decodedImage())
        let request = try request(path: "reappear.png")

        await model.load(request: request, loader: loader)
        XCTAssertEqual(model.phase.kind, .cancelled)
    ...
    func testChangingLoadTokenForcesSameIdentityRetry_UI_PT_020() async throws {
        let model = FoveaImageModel()
        let loader = RetryImageLoader(image: try decodedImage())
        let request = try request(path: "retry-token.png")
        let firstToken = UUID()

        await model.load(
            request: request,
            loader: loader,
            loadToken: firstToken
        )
        XCTAssertEqual(model.phase.kind, .failure)
...
private actor CancellationThenSuccessImageLoader: ImageLoading {
    let image: DecodedImage
    private(set) var requestCount = 0
    ...
    func image(for request: ImageRequest) async throws -> DecodedImage {
        requestCount += 1
        if requestCount == 1 { throw CancellationError() }
        return image
    }
}
