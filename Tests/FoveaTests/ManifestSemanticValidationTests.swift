...
final class ManifestSemanticValidationTests: XCTestCase {
    ...
    func testOriginalManifestEntryAboveRuntimeLimitIsReconciled_SEC_CASE_030() async throws {
        ...
        keyMaterial.append(Data(oversizedDigest.utf8))
        let newKey = SHA256.hash(data: keyMaterial)
        ...
        let reconciledData = try Data(contentsOf: fileURL)
        XCTAssertNotEqual(reconciledData, corrupted)
        let reconciled = try jsonObject(reconciledData)
        XCTAssertTrue(try dictionary(reconciled["entries"]).isEmpty)
        let blobFiles = try FileManager.default.contentsOfDirectory(
            at: root.appendingPathComponent("blobs", isDirectory: true),
            includingPropertiesForKeys: nil
        )
        XCTAssertTrue(blobFiles.filter { !$0.lastPathComponent.hasPrefix(".akashic-") }.isEmpty)
    }
