...
final class ProfileAccessPolicyTests: XCTestCase {
    ...
    func testDestinationPolicyRejectsBeforeCacheAndNetwork_AUTH_PT_017() async throws {
        let encoded = AccessSpyEncodedStore()
        let records = AccessSpyRecordStore()
        let transport = AccessSpyTransport()
        let allowedOrigin = try HTTPOrigin(
            url: XCTUnwrap(URL(string: "https://images.example.test/allowed.png"))
        )
        ...
        let denied = try ImageRequest.publicImage(
            ...
            appID: "destination-policy-tests",
            cachePolicy: .onlyIfCached
        )

        do {
            _ = try await pipeline.image(for: denied)
            XCTFail("Destination ACL must reject before cache and transport")
        } catch let failure as PipelineFailure {
            XCTAssertEqual(failure.reasonCode, "profile-access-denied")
            XCTAssertEqual(failure.stage, .requestValidation)
        }
...
private actor AccessSpyEncodedStore: OriginalEncodedStoring {
    ...
    func commit(data: Data, contentID: String, namespace: String) throws -> StoredBlob {
        operationCount += 1
        return try StoredBlob(physicalID: PhysicalBlobID(), byteCount: data.count, wasCreated: true)
    }
