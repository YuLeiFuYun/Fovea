...
final class HTTPConformanceTests: XCTestCase {
    ...
    func testWPTVaryMatrix_HTTP_CONF_WPT_VARY() throws {
        ...
        run("headers not named by Vary are ignored") {
            XCTAssertEqual(
                select([ignoreOtherRecord], for: fooOneOtherThree)?.contentID,
                ignoreOtherRecord.contentID)
        }
        ...
        run("three-way Vary matches all fields") {
            XCTAssertEqual(
                select([threeWayRecord], for: threeWay)?.contentID, threeWayRecord.contentID)
        }
    ...
    private func run304Scenario(
        ...
        let request = try ImageRequest.publicImage(
            url: try XCTUnwrap(
                URL(string: "https://example.test/wpt-304-\(UUID().uuidString).png")),
    ...
    private func pipelineScenario(
        ...
        let request = try ImageRequest.publicImage(
            url: try XCTUnwrap(
                URL(string: "https://example.test/wpt-status-\(UUID().uuidString).png")),
            target: try TargetPixels(width: 20, height: 20),
            appID: "tests"
        )
