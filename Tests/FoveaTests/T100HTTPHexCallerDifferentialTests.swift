...
final class T100HTTPHexCallerDifferentialTests: XCTestCase {
    func testBoundedStagingDigestMatchesIndependentSHAReferenceInMemoryAndSpilled_HTTP_HEX_PT_001()
        throws
    {
        ...
        for threshold in [data.count + 1, 0] {
            let directory = FileManager.default.temporaryDirectory
                .appendingPathComponent(
                    "fovea-t100-http-hex-\(UUID().uuidString)", isDirectory: true)
    ...
    func
        testDestinationPolicyFingerprintMatchesIndependentSortedOriginSHAReference_HTTP_HEX_PT_002()
        throws
    {
    ...
    func
        testURLSessionPolicyFingerprintMatchesIndependentPolicyMaterialSHAReference_HTTP_HEX_PT_003()
        throws
    {
        let origins: Set<HTTPOrigin> = [
