...
final class MemoryCacheTests: XCTestCase {
    ...
    func testCompactSieveContainsDoesNotPromoteVisitState_CACHE_PT_053() {
        let queried = FoveaCompactSieveCache<Int, Int>(costLimit: 2)
        queried.insert(1, for: 1, cost: 1)
        queried.insert(2, for: 2, cost: 1)
        XCTAssertTrue(queried.contains(1))
        XCTAssertFalse(queried.contains(99))
        queried.insert(3, for: 3, cost: 1)
        XCTAssertFalse(queried.contains(1))
        XCTAssertTrue(queried.contains(2))
        XCTAssertTrue(queried.contains(3))

        let visited = FoveaCompactSieveCache<Int, Int>(costLimit: 2)
        visited.insert(1, for: 1, cost: 1)
        visited.insert(2, for: 2, cost: 1)
        XCTAssertEqual(visited.value(for: 1), 1)
        visited.insert(3, for: 3, cost: 1)
