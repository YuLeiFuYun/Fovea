...
final class ImageDecoderTests: XCTestCase {
    func testPreparedProbeMatchesStandardProbe_DIAG_PT_013() throws {
        ...
        XCTAssertEqual(preparation.probe, expected)
    }
...
extension ImageDecoderTests {
    ...
    func testPreparedDecodeMatchesDirectDecodeDiagPt014() throws {
        ...
        XCTAssertEqual(prepared.pixelWidth, direct.pixelWidth)
        XCTAssertEqual(prepared.pixelHeight, direct.pixelHeight)
        XCTAssertEqual(prepared.colorDescription, direct.colorDescription)
        XCTAssertEqual(prepared.alphaMode, direct.alphaMode)
        XCTAssertEqual(prepared.pixelFormat, direct.pixelFormat)
    }
    ...
    func testDecodeStageRejectsUnsupportedCapabilityAndDiscardsPreparation() async throws {
        ...
        let request = try ImageRequest.publicImage(
            url: XCTUnwrap(URL(string: "https://example.test/codec-contract.png")),
        ...
        do {
            ...
            XCTFail("不支持探测格式的后端不得进入像素解码")
        } catch let failure as PipelineFailure {
            XCTAssertEqual(failure.category, .decode)
            XCTAssertEqual(failure.stage, .decode)
            XCTAssertEqual(failure.reasonCode, "unsupported-codec-capability")
        }
    ...
    func testBackendWorkingSetUnderreportCannotBypassGenericAdmission() async throws {
        ...
        let stage = DecodeStage(
            ...
            diagnostics: NullDiagnosticsSink(),
            maximumConcurrentDecodes: 1,
            maximumDecodeWorkingSetBytes: 64,
            maximumQueuedDecodes: 1
        )
        let request = try ImageRequest.publicImage(
            url: XCTUnwrap(URL(string: "https://example.test/underreport.png")),
            target: TargetPixels(width: 10, height: 10),
            appID: "codec-contract-tests"
        )
        ...
        do {
            ...
            XCTFail("后端低报不得把通用工作集下界压缩到资源上限以内")
        } catch let failure as PipelineFailure {
    ...
    ImageCodec, @unchecked Sendable
{
    ...
    init(capabilities: ImageCodecCapabilities, backendWorkingSetBytes: Int) {
        self.codecDescriptor = ImageCodecDescriptor(
            identifier: ImageCodecIdentifier(rawValue: "test.contract-prepared"),
    ...
    func prepare(data: Data, limits: DecodeLimits) throws -> ImageDecodePreparation {
        ...
        prepared += 1
        lock.unlock()
        return ImageDecodePreparation(probe: try probe(data: data, limits: limits))
    }
    ...
    func snapshot() -> Snapshot {
        lock.lock()
        defer { lock.unlock() }
        return Snapshot(prepared: prepared, discarded: discarded, decoded: decoded)
    }
}
