...
final class CacheRefreshTransactionTests: XCTestCase {
    ...
    func testCancellationAfterSameVariant200CommitRestoresPreviousRecord_CACHE_PT_039()
        async throws
    {
        let request = try ImageRequest.publicImage(
            url: XCTUnwrap(URL(string: "https://example.test/cancelled-200-refresh.png")),
            target: TargetPixels(width: 20, height: 20),
            appID: "refresh-cancellation"
        )
        let oldBody = try makePNG(width: 20, height: 20, red: 31)
        let newBody = try makePNG(width: 20, height: 20, red: 32)
        let oldContentID = ContentID(data: oldBody)
        let old = RepresentationRecord(
            securityNamespace: request.namespace.value,
            namespaceGeneration: 0,
            baseKeyDigest: request.fetchBaseKey.digestHex,
            ...
            lastModified: nil,
            disposition: .reusable,
            contentID: oldContentID.description,
            payloadLength: oldBody.count,
            contentType: "image/png"
        )
        let records = GatedRefreshRecordStore(initial: old)
        await records.gateNextPut()
        let pipeline = FoveaPipeline(
            transport: RefreshOKTransport(body: newBody),
...
private actor RefreshEncodedStore: OriginalEncodedStoring {
    ...
    func commit(data: Data, contentID: String, namespace: String) throws -> StoredBlob {
        try StoredBlob(physicalID: PhysicalBlobID(), byteCount: data.count, wasCreated: false)
    }
...
private actor RefreshOKTransport: HTTPTransporting {
    nonisolated let reusePolicy = TransportReusePolicy.reusable(
        contextIdentifier: "refresh-ok-transaction-test"
    )
    ...
    func execute(_ request: TransportRequest) async throws -> TransportResponse {
        TransportResponse(
            head: try TransportResponseHead(
                statusCode: 200,
                headers: [
                    "Content-Type": "image/png", "Cache-Control": "max-age=3600", "ETag": "new",
                ],
                url: request.request.url
            ),
            body: body,
