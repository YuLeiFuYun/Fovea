...
final class CacheGarbageCollectionTests: XCTestCase {
    func testBootstrapRemovesTemporaryAndOrphanBlobFilesCachePt013() async throws {
        ...
        let reopened = try await reopenAkashicOriginalEncodedStore(root: storeRoot)

        XCTAssertFalse(FileManager.default.fileExists(atPath: orphan.path))
        XCTAssertFalse(FileManager.default.fileExists(atPath: temporary.path))
        let retained = blobs.appendingPathComponent(stored.physicalID.foveaStorageFileName)
    ...
    func testStoredContentReferenceRejectsNoncanonicalIdentity() throws {
    ...
    func testGarbageCollectionRetainsBlobUntilLastVaryReferenceIsRemovedCachePt020() async throws {
        ...
        let english = try HTTPVarySelection(
            fieldNames: ["accept-language"],
            values: ["accept-language": .field("en")]
        )
        let french = try HTTPVarySelection(
            fieldNames: ["accept-language"],
    ...
    func testGarbageCollectionRecoversBlobPublishedBeforeRecordAfterReopenCachePt013() async throws
    {
        ...
        var encoded: AkashicOriginalEncodedStore? = try await AkashicOriginalEncodedStore.open(
            root: encodedRoot
        )
        _ = try await encoded!.commit(
            data: data,
        ...
        let reopenedEncoded = try await reopenAkashicOriginalEncodedStore(root: encodedRoot)
