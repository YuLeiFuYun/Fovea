...
package enum BenchmarkHarnessError: Error, Equatable, Sendable {
    case invalidFixtureURL
    case outputExceedsTarget
}

/// 在确定性来源上执行代表性工作负载，并生成可验证的烟雾基准工件。
/// 这些结果用于回归检测，不替代真机 Instruments、能耗或热状态测量。
package enum BenchmarkSmokeHarness {
    package static func runW1FeedScroll(outputDirectory: URL) async throws -> BenchmarkSmokeArtifact
    {
    ...
    package static func runW2DetailHero(outputDirectory: URL) async throws -> BenchmarkSmokeArtifact
    {
        let fixtures = try [
        ...
        for (sourceIndex, resource) in resources.enumerated() {
            for target in targets {
                ...
                do {
                    let image = try await pipeline.image(for: request)
                    guard image.pixelWidth <= target.width, image.pixelHeight <= target.height
                    else {
