    ...
    private final class GalleryModel: ObservableObject {
        ...
        @Published private(set) var externalNetworkingAuthorized = false
        @Published var networkMode: NetworkMode = .interactive
        @Published var requestGeneration: UInt64 = 0

        private var isStarting = false

        func authorizeExternalNetworkingAndStart() async {
            guard pipeline == nil, !isStarting else { return }
            externalNetworkingAuthorized = true
            isStarting = true
            ...
            do {
                ...
                startupFailure = Self.safeMessage(for: error)
            }
        }
    ...
    private struct DemoImage: Identifiable {
        ...
        private static let catalog = [
            CatalogEntry(
                id: "picsum-landscape",
                title: "响应式远景",
                url: "https://picsum.photos/seed/fovea-gallery-landscape/1200/800",
                contentMode: .fill
            ),
            CatalogEntry(
                id: "picsum-portrait",
                title: "响应式人像比例",
                url: "https://picsum.photos/seed/fovea-gallery-portrait/720/1080",
                contentMode: .fill
            ),
            CatalogEntry(
                id: "httpbin-png",
        ...
        static let gallery: [DemoImage] = catalog.compactMap { entry in
            ...
            NavigationView {
                Group {
                    if let pipeline = model.pipeline {
                        ...
                        .padding(24)
                    } else if model.externalNetworkingAuthorized {
                        ...
                        VStack(spacing: 14) {
                            ...
                            Text("此示例会访问第三方图片服务")
                                .font(.headline)
                            Text("启动后将向 picsum.photos 与 httpbin.org 发出请求。默认不联网；仅在你明确同意后加载远程示例。")
                                .multilineTextAlignment(.center)
                                .foregroundStyle(.secondary)
                                .frame(maxWidth: 520)
                            Button("允许外部网络并启动") {
                                Task { await model.authorizeExternalNetworkingAndStart() }
                            }
