...
@Test("All memory contestants preserve basic value and cost semantics")
func memoryBasicSemantics() async throws {
...
@Test("Fovea SIEVE resists a one-hit scan at least as well as specialist caches")
func memoryScanResistance() async throws {
...
@Test("All memory contestants remain bounded and coherent under concurrent mixed access")
func memoryConcurrentCorrectness() async throws {
    for kind in MemoryContestantKind.allCases {
...
@Test("Fovea and PIN disk stores persist, converge after external deletion, and reject corruption")
func diskAdversarialCorrectness() async throws {
...
@Test("Fovea and PIN disk stores return exact bytes across a mixed-size corpus")
func diskMixedCorrectness() async throws {
    for kind in DiskContestantKind.allCases {
        ...
        let result = try await CacheLabWorkloads.runDiskMixed(
            ...
            root: root,
            objectsPerSize: 2,
            sizes: [4_096, 65_536, 262_144]
        )
        #expect(result.checks.allSatisfy { $0.passed }, "\(kind.rawValue) mixed disk checks")
    }
}
@Test("Durable PIN wrapper hides native objects until a valid proof is published")
func durablePINProofControlsVisibility() async throws {
    let root = try temporaryDirectory("pin-proof-visibility")
    defer { try? FileManager.default.removeItem(at: root) }
    let cache = try PINDurableValidatedDiskContestant(root: root)
    let value = CacheLabWorkloads.payload(seed: 91, count: 8_192)
...
@Test("Disk contestant roles prevent native PIN writes from entering the D5 ranking")
func diskSemanticStratification() {
