...
public final class FoveaMemoryContestant: MemoryCacheContestant, @unchecked Sendable {
    ...
    private let cache: MemoryCache<String, Data>

    public init(costLimit: Int) {
        cache = MemoryCache(costLimit: costLimit)
    }

    public func value(for key: String) async -> Data? { cache.value(for: key) }
    public func insert(_ value: Data, for key: String, cost: Int) async {
        cache.insert(value, for: key, cost: cost)
    }
    public func removeValue(for key: String) async { cache.remove(key) }
    public func removeAll() async { cache.removeAll() }
    public func count() async -> Int { cache.count }
    public func currentCost() async -> Int { cache.currentCost }
}
...
public final class LRUCacheContestant: MemoryCacheContestant, @unchecked Sendable {
    ...
    public func value(for key: String) async -> Data? { cache.value(forKey: key) }
    public func insert(_ value: Data, for key: String, cost: Int) async {
        cache.setValue(value, forKey: key, cost: cost)
    }
    public func removeValue(for key: String) async { cache.removeValue(forKey: key) }
    public func removeAll() async { cache.removeAll() }
    public func count() async -> Int { cache.count }
    public func currentCost() async -> Int { cache.totalCost }
}
...
public final class PINMemoryContestant: MemoryCacheContestant, @unchecked Sendable {
    ...
    public func removeValue(for key: String) async { bridge.removeData(forKey: key) }
    public func removeAll() async { bridge.removeAllData() }
    public func count() async -> Int { Int(bridge.count) }
    public func currentCost() async -> Int { Int(bridge.totalCost) }
}
...
public final class FoveaDiskContestant: DiskCacheContestant, @unchecked Sendable {
    ...
    public static func open(root: URL, softLimitBytes: Int = 512 * 1_024 * 1_024) async throws
        -> FoveaDiskContestant
    {
    ...
    public func readExceptionCount() async -> Int { 0 }
...
public final class PINDiskContestant: DiskCacheContestant, @unchecked Sendable {
    ...
    public func removeValue(for key: String) async throws { bridge.removeData(forKey: key) }
    public func removeAll() async throws { bridge.removeAllData() }
    public func fileURL(for key: String) async throws -> URL? { bridge.fileURL(forKey: key) }
    public func readExceptionCount() async -> Int { Int(bridge.readExceptionCount) }
    public func reopen() async throws -> any DiskCacheContestant {
        PINDiskContestant(root: root, cacheName: cacheName)
    }
}
