...
CAPTURE = PERFORMANCE / "capture_w5_apng_owned_swift_playback.py"
VALIDATOR = PERFORMANCE / "validate_w5_apng_owned_swift_playback.py"
ARTIFACTS = ROOT / ".artifacts/performance"


class W5APNGOwnedSwiftPlaybackTests(unittest.TestCase):
    ...
    def test_capture_validates_and_tampering_fails_closed(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix="w5-apng-owned-swift-contract-", dir=ARTIFACTS
        ) as temporary:
            ...
            self.assertTrue(report["allSwiftPythonExact"])
            self.assertTrue(report["allSwiftAppleExactWhereAvailable"])
            ...
            frame_path = output / "swift/APNG-SUBRECT-NONE/frame-001.rgba"
            original_frame = frame_path.read_bytes()
            altered = bytearray(original_frame)
            altered[0] ^= 1
            frame_path.write_bytes(altered)
            tampered_frame = self.validate(output)
            self.assertNotEqual(tampered_frame.returncode, 0)
            self.assertIn(
                "retained Swift frame is not reproducible",
            ...
            self.assertIn(
                "unexpected owned Swift playback root artifact",
