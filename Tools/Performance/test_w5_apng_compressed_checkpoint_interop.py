...
CAPTURE = PERFORMANCE / "capture_w5_apng_compressed_checkpoint_interop.py"
VALIDATOR = PERFORMANCE / "validate_w5_apng_compressed_checkpoint_interop.py"
ARTIFACTS = ROOT / ".artifacts/performance"


class W5APNGCompressedCheckpointInteropTests(unittest.TestCase):
    ...
    def test_capture_validates_and_tampering_fails_closed(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix="w5-apng-compressed-interop-contract-",
            dir=ARTIFACTS,
        ) as temporary:
            ...
            self.assertTrue(report["allPythonToSwiftDecodedExact"])
            self.assertTrue(report["allSwiftToPythonDecodedExact"])

            original_report = report_path.read_bytes()
            report["fixtureCount"] += 1
            ...
            swift_blob = output / "fixtures/STRUCTURED-16X16/swift.fapc"
            original_blob = swift_blob.read_bytes()
            altered = bytearray(original_blob)
            altered[-1] ^= 1
            swift_blob.write_bytes(altered)
            tampered_blob = self.validate(output)
            self.assertNotEqual(tampered_blob.returncode, 0)
            self.assertIn(
                "retained Swift blob is not reproducible",
                tampered_blob.stdout + tampered_blob.stderr,
            )
            swift_blob.write_bytes(original_blob)
            ...
            self.assertIn(
                "unexpected interop root artifact",
