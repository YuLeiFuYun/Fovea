    ...
    private func waitForActive(
        ...
        try await waitUntil(
            timeoutNanoseconds: 2_000_000_000, error: .activeDisplayLinkDidNotAdvance
        ) {
    ...
    private func captureHidden(
        ...
        guard end.presentation.acceptedTargetCount == settled.presentation.acceptedTargetCount
        else {
    ...
    private func driverModeName(_ mode: AnimationPlaybackSchedulingMode) -> String {
        mode == .externalPresentationTicks
            ? "external-presentation-ticks" : "automatic-deadline-loop"
    }
    ...
    func makeFrame(index: Int) -> DecodedImage {
        ...
        let colorSpace =
            native ? CGColorSpace(name: CGColorSpace.sRGB)! : CGColorSpaceCreateDeviceRGB()
        let bitmapInfo =
            native
    ...
    @main
    enum FoveaAnimationMacLabUnsupported {
        static func main() {
            print("FoveaAnimationMacLab 仅支持 macOS。")
        }
    }
#endif
