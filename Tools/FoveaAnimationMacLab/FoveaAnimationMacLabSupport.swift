    ...
    final class StrictDispatchSleepState: @unchecked Sendable {
        ...
        func install(
            ...
            timer.schedule(
                deadline: DispatchTime(uptimeNanoseconds: deadline), leeway: .nanoseconds(0))
    ...
    struct CallbackTimingEvent {
        ...
        var json: [String: Any] {
            [
                "sequence": sequence, "elapsedNanoseconds": elapsedNanoseconds,
    ...
    final class CallbackTimingRecorder {
        ...
        func record(frameIndex: Int, timestamp: UInt64) {
            ...
            if let previous = observations.last?.frameIndex {
                let delta =
                    frameIndex > previous
                    ? frameIndex - previous : frameCount - previous + frameIndex
    ...
    struct PlaybackFixture {
        ...
        static func make(options: Options) async throws -> PlaybackFixture {
            ...
            let predecodeAll =
                ProcessInfo.processInfo.environment["FOVEA_MACLAB_PREDECODE_ALL"] == "1"
            let runtime = AnimationPlaybackRuntime(
